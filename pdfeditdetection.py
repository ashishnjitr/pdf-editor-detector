# -*- coding: utf-8 -*-
"""PDFEDITDEtection.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1dxp4AAsPoumr6zjlpb1GUPd541xPCKQv
"""
...
def analyze_pdf(file_bytes):
    ...
    if len(eof_markers) > 1:
        flags.append(f"⚠️ Multiple file versions detected ({len(eof_markers)} saves). This strongly indicates post-creation editing.")
    ...
    try:
        ...
        if metadata:
            cleaned_meta = {k.replace('/', ''): v for k, v in metadata.items()}
            ...
            if create_date and mod_date and (create_date != mod_date):
                flags.append("⚠️ The Modification Date does not match the Creation Date.")

            producer = cleaned_meta.get("Producer", "").lower()
            suspicious_tools = ["ilovepdf", "smallpdf", "pdf2go", "nitro", "soda", "libreoffice", "canva"]
            for tool in suspicious_tools:
                if tool in producer:
                    flags.append(f"⚠️ Document processed by known consumer editing tool: '{cleaned_meta.get('Producer')}'")
        else:
            flags.append("⚠️ No metadata found. The file may have been flattened to hide history.")
    except Exception as e:
        flags.append(f"❌ Error parsing metadata: {str(e)}")

    # Print Results
    print("\n" + "="*40)
    print("🔍 PDF FORENSIC REPORT")
    print("="*40)
    if flags:
        print("\n".join(flags))
    else:
        print("✅ No obvious modifications or edit signatures detected.")
    print("="*40)

# 3. Trigger the file uploader window
