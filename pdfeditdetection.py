    ...
    def audit_h1b_copy(file_bytes):
        ...
        if len(eof_markers) > 1 or len(xref_markers) > 1:
            ...
            audit["tamper_evidence"].append(
                f"Multi-Save Container: Found {len(eof_markers)} %%EOF markers. PDF was appended/re-saved after initial creation."
            )
        ...
        try:
            ...
            for tool in consumer_tools:
                if tool in combined_meta:
                    ...
                    audit["risk_score"] += 50
                    audit["tamper_evidence"].append(f"Editor Footprint Trapped: File generated or saved using '{tool.upper()}'. Official notices are not generated by web converters.")
        except Exception:
            pass
        ...
        try:
            ...
            for page_num in range(len(doc)):
                ...
                # Extract USCIS Receipt Number Pattern: 3 letters (EAC, WAC, LIN, SRC, IOE, MSC) + 10 digits
                ...
                if receipt_matches:
                    ...
                    if loose_receipt:
                        ...
                        audit["risk_score"] += 40
                        audit["tamper_evidence"].append(f"Suspicious Receipt Pattern: '{loose_receipt[0]}' does not match standard 13-digit USCIS structure.")

                # Extract Validity Date Windows (e.g., Valid from MM/DD/YYYY to MM/DD/YYYY)
                ...
                # Inspect Text Blocks & Layout Overlays
            ...
            if len(suspicious_fonts) > 1 and len(eof_markers) > 1:
                audit["risk_score"] += 30
                audit["tamper_evidence"].append(
                    f"Typographic Font Discrepancy: Isolated non-standard fonts [{', '.join(suspicious_fonts[:2])}] overlaid alongside standard USCIS typography."
                )

        except Exception as e:
            audit["tamper_evidence"].append(f"Audit Interruption: {str(e)}")

        if audit["risk_score"] >= 50 or audit["inferred_tool"] != "None Detected":
            audit["is_tampered"] = True
    ...
    if h1b_file is not None:
        ...
        if result["is_tampered"]:
            st.error(
                ...
                f"Tamper Confidence Score: {min(result['risk_score'], 100)}/100 — Document shows structural, typographic, or external software alterations.",
                icon="🛑"
            )
        elif result["risk_score"] > 20:
            st.warning(
                f"⚠️ **H-1B VERDICT: CAUTION (INCONSISTENCIES FOUND)** \n\n"
                f"Risk Score: {result['risk_score']}/100 — Multi-save or layout deviations found. Manual cross-examination recommended.",
                icon="⚡"
            )
        else:
            st.success(
                "🛡️ **H-1B VERDICT: STRUCTURALLY CLEAN / VERIFIED SCAN** \n\n"
                "Document matches single-compilation structure with no external editing tool overlays detected.",
                icon="✅"
            )
